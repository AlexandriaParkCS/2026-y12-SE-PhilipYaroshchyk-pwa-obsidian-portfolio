***Level 0 Data Flow Diagram***

The Level 0 Data Flow Diagram (Context Diagram) that identifies the external entities that interact with the application and their interfaces is shown below:
* Users (external entities) interface with the BreakingBank Web application by registering, logging in and interacting with the application by entering the income and expense entries and interacting with application's Dashboards and Analytics
* BreakingBank application performs Authentication and Authorisation based on the user credentials
* BreakingBank application generates budget planning dashboards
* The BreakingBank application is connected to the Database to store and retrieve user data

```mermaid
--- 
title: BreakingBank Context Diagram
---
flowchart

User
BreakingBank((BreakingBank))
Database

User -- Credentials, Transactions, Goals --> BreakingBank 
BreakingBank -- Dashboards --> User

BreakingBank -- User Onboarding --> Database

BreakingBank -- Entry Transactions and Goals --> Database
Database -- Filtered and Aggregated Data for Dashboards & Summaries--> BreakingBank
```

***The Level 1 Data Flow Diagram***

This section contains the Level 1 Data Flow Diagrams. The Login/registration functionality is shown first:
* New users provide their details for registration (including username, email and password), which BreakingBad validates and saves in the database (password hash is stored instead of the password value) 
* Registered uses provide their credentials, which BreakingBad verifies against the saved user data. 
* If Registration/Login is successful, BreakingBad Authenticates the user by creating a Flask session and Authorises application access

```mermaid
---
title: Level 1 Data Flow Diagram (Login/Registration)
---
flowchart

User
Register((Login/Register))
Dashboards((Dashboard<br>Generation))

Database[(Database)]

User -- Credentials/User Details --> Register

Register <-- Verify/Create --> Database

Register -- Authorise --> Dashboards
Dashboards --> User
```

Data entry part of the application functionality is explained below:
* Authorised users enter their expense and income records in the BreakingBank application UI
* BreakingBank application inserts those records in the SQLite database 

```mermaid
---
title: Level 1 Data Flow Diagram (Transaction entry)
---
flowchart

AuthorisedUser[Authorised User]
DataInsert((Data Insert))
Database[(Database)]

AuthorisedUser -- Transactions (Income and Expenses) --> DataInsert
DataInsert -- Save transactions --> Database
```

The following flow diagram explains the User interaction Goals:
```mermaid
---
title: Level 1 Data Flow Diagram (Financial goals entry and edit)
---
flowchart

AuthorisedUser[Authorised User]
DataInsert((Data Insert))
Database[(Database)]

AuthorisedUser -- Create or edit goals --> DataInsert
DataInsert -- Save goals --> Database
```


The flow diagram corresponding to the User interaction with the Dashboards and Summaries is shown below:
* Authorised users can access the Dashboards generated by the application.
* Dashboards contain a list of recent expenditures and recent income entries and a list of goals. Clicking on each of the goals can navigate the user to the goal's Summary Page 
* Application fetches the transaction and goals data from the database. Application strictly authorises users to only have access to their own data.
* Once dashboards are generated, they are displayed to the user.
* User can interact with the dashboards and summaries by focusing on their individual elements: Charts, Tables, etc

```mermaid
---
title: Level 1 Data Flow Diagram (Dashboard and Summmaries Viewing)
---
flowchart

AuthorisedUser[Authorised User]
Dashboards
Summaries
DashboardGeneration((Analytics<br>Generator))
Database[(Database)]

AuthorisedUser -- Interacts --> Dashboards
AuthorisedUser -- Interacts --> Summaries

Summaries -- Tables/Charts/<br>Aggregation/Analytics <--> DashboardGeneration
Dashboards -- Expenses/Income/Goals <--> DashboardGeneration
DashboardGeneration -- Transactions and Goals <--> Database

Dashboards -- Displayed --> AuthorisedUser
Summaries -- Displayed --> AuthorisedUser
```